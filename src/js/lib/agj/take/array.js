define(['../core', '../take'], function (agj, take) {
	...
	function nextTo(arr, item, dontWrapAround) {
		var index = module.array.nextOrPrevIndex(arr, arr.indexOf(item), true, dontWrapAround);
		if (index >= 0)
			return arr[index];
		return agj.undefined;
	}
	function prevTo(arr, item, dontWrapAround) {
		var index = module.array.nextOrPrevIndex(arr, arr.indexOf(item), false, dontWrapAround);
		if (index >= 0)
			return arr[index];
		return agj.undefined;
	}
	function nextIndex(arr, index, dontWrapAround) {
		return module.array.nextOrPrevIndex(arr, index, true, dontWrapAround);
	}
	function prevIndex(arr, index, dontWrapAround) {
		return module.array.nextOrPrevIndex(arr, index, false, dontWrapAround);
	}
	function nextIndexTo(arr, item, dontWrapAround) {
		return module.array.nextOrPrevIndex(arr, arr.indexOf(item), true, dontWrapAround);
	}
	function prevIndexTo(arr, item, dontWrapAround) {
		return module.array.nextOrPrevIndex(arr, arr.indexOf(item), false, dontWrapAround);
	}
